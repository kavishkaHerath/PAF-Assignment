			...
			if (AUTHENTICATION_HEADER != null && AUTHENTICATION_HEADER.size() > 0) {
				...
				if (m.isAnnotationPresent(RolesAllowed.class)) {
					...
					if (user_role.contains("admin")) {
						wt = c.target("http://localhost:8083/UserAuthifiation/Authifiation").path("usersAuth");
					} 
					else if (user_role.contains("patient")) {
						wt = c.target("http://localhost:8083/UserAuthifiation/Authifiation").path("usersAuth");

					} else {
						wt = c.target("http://localhost:8083/UserAuthifiation/Authifiation").path("usersAuth/deny");
					}
